...
public class ContactHelper {
    ...
    public static List<ContactInfo> contactFilter(String filterStr, List<ContactInfo> contactInfoList){
    ...
    public static List<String> setupLetterIndexList(List<ContactInfo> contactInfoList){
        ...
        for (ContactInfo contactInfo : contactInfoList){
            ...
            if (!found && "#".equals(firstLetter)){     //找到#字符就不在判断
                found = true;
            }
        }
    ...
    public static List<ContactInfo> setupContactInfoList(String[] contacts){
        ...
        for (String contact : contacts){
            ...
            pinyinName = pinyinName.toUpperCase();      //若包含英文字母则额外进行大写处理
    ...
    private static String setupSortLetters(String contact) {
        String firstChar = String.valueOf(contact.charAt(0));   //获取第一个字母
        int mode = PinyinUtils.CASE_UPPERCASE | PinyinUtils.TRIM_NON_CHAR;
        String pinyin = PinyinUtils.toPinyinString(firstChar, mode);
        if (!TextUtils.isEmpty(pinyin)){    //首个字符是汉字，返回拼音
            return pinyin;
        }else {
            String words = contact.split("[^a-zA-Z]")[0];   //或者以英文开头
